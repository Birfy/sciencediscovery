...
|---|---|---|
| 对话放在哪 | API 的记录，每一步重新发 | JiuwenSwarm 的会话，每个智能体一个；API 的记录只用来启动空会话 |
...
2. **历史与上下文。** JiuwenSwarm 把每个智能体的对话保存在稳定的会话里并自己压缩；API 的记录只用来启动 JiuwenSwarm 还没有上下文的会话。运行契约和每一步的动态上下文（计划快照、持久状态）没有注入，见“上下文管理”。如果 API 的记录和 JiuwenSwarm 的会话出现分歧（在 API 里编辑或回退了对话），两边不会被对齐。
