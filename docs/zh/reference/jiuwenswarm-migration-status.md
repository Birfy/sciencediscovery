...
内置循环在每个模型步骤都会组装上下文并管理大小。在 JiuwenSwarm 执行器上：

| 内置循环 | JiuwenSwarm 执行器 |
|---|---|
| 系统提示各段（身份、治理、能力、技能） | 文本相同，每次运行组装一次 |
| 运行契约（受保护，每一步都在） | **没有注入** |
| 每一步的动态上下文：计划快照、持久状态、插件贡献、`runtime_context_data` | **没有注入**；模型只能从之前的工具结果里看到计划和状态 |
| 工具路由提示 | **没有注入** |
| 工具输出守卫和读取（超大结果按引用） | 可用：调用走同一个 `ToolRegistry` |
| 按模型上下文窗口计算的预算 | **没有** |
| 历史压缩（总结较早的轮次） | **没有** |
| 输入过长错误的恢复（压缩后重试） | **没有**：运行以供应商的原话失败 |
| 轨迹与 evidence 记录 | **没有** |

JiuwenSwarm 自己有一套上下文处理；它在这里用到的模型上表现如何，还没有测过。

## 已知缺口

1. **没有 evidence / 轨迹。** JiuwenSwarm 运行不产生 `agent.record`/`evidence`，轨迹视图为空。
2. **历史。** 每次运行都会把 API 的记录一并发给适配器，由模型代理插入请求；JiuwenSwarm 每一轮在自己的独立会话里运行。所以在内置循环上开始的会话、恢复的子代理都能延续（内置循环已经压缩过的历史保持压缩）。**这个执行器上不会做新的压缩、上下文预算和溢出恢复**，运行契约以及每一步的动态上下文（计划快照、持久状态、插件上下文）也没有注入，见下面“上下文管理”。JiuwenSwarm 自己的会话存储不保存对话状态。
