...
11. **UT 已在真实 JiuwenSwarm 下审计。** `services/api/src/server.test.ts`（host 档 UT 里最大的测试文件）现在通过 `scripts/with-jiuwenswarm.sh` 让智能体轮次跑在真实的 JiuwenSwarm 和适配器上（`pnpm ci:ut:host` 已接好）。审计发现：
    ...
    - **第十三个发现，做了缓解，没有根治（根因在上游）：** 没有把 gap 9 那个"偶发不可见"的 MCP 工具调用停在"难以定位"上，而是继续往下查。已经能按需复现——在一个真实的本地 JiuwenSwarm 实例上（`scripts/with-jiuwenswarm.sh` 自己的设计就是整条命令共享一个实例和 adapter）同时跑好几份 "native MCP literature flow produces an audited cited summary"，外加一些无关测试文件的负载，12 次里失败了 7 次。先单独跑了 25 次，全部干净通过，确认了这个问题需要共享实例上真实的并发压力才会触发，不是一个确定性的代码 bug。找到了三种不同的失败表现，全都是同一个共享实例并发争用的下游结果：(a) 最轻的一种，和 CI 实际命中的一致：运行成功完成，最终答案也对（引用的 PMID 是对的，说明工具确实执行过），但 `mcp__pubmed__search` 的 SSE `tool.started` 事件压根没出现——工具显然跑了，但桥接层自己上报这次调用的事件丢了；(b) JiuwenSwarm 自己的 `resource_mgr` 报 `Ability not found in resource_mgr: tool_search`，即便 `offerDeferredTools` 明明刚刚为这次运行 promote 过它——模型压根发现不了 `mcp__pubmed__search`，运行在还没发出 MCP 权限请求之前就结束了；(c) 最严重的一种：`[jiuwenswarm-agent] could not answer JiuwenSwarm's approval question call-pubmed: HTTP 404`——这个进程回传给 JiuwenSwarm 的审批决定直接被拒——之后运行彻底沉默，直到 ScienceDiscovery 自己的空闲超时看门狗在四分钟后把它杀掉（`Agent run stalled: no gateway progress for 240000 ms`），和 gap 10、第十一条发现里记录的那个嵌套 `task` 卡顿，失败特征一模一样。这三种表现指向同一件事：JiuwenSwarm 0.2.6 每个实例自己的注册状态和审批问题状态，在多会话并发访问下并不可靠——这是 `scripts/with-jiuwenswarm.sh` 故意选用的共享实例设计（整条命令一个实例，"和产品自己的会话一样"）真实存在的可靠性边界，不是能枚举成一份固定测试清单、或者靠放宽某条断言就能修掉的 bug（放宽断言只会把一个真实的、哪怕是上游的失败给掩盖掉）。没有继续往下追，而是做了缓解：`pnpm --recursive`（也就是 `workspace-packages` 这个 UT workload）现在把并发度限制在 `--workspace-concurrency=2`（pnpm 自己的默认值是 4，也就是每个 CPU 跑一个包的测试），让能同时挤到共享实例注册/审批状态上的包数量减半。`.ci/ci-contract.mjs` 里重新推导这条命令的部分也同步更新了，`pnpm ci:selftest` 还是会精确校验这条命令的形状。这只是缩小了窗口，没有彻底关上：真正的修复要么要 JiuwenSwarm 0.2.6 自己加固这部分状态在并发访问下的安全性，要么要 `scripts/with-jiuwenswarm.sh` 改成每个包一个独立实例（改动大得多，而且违背它自己"一个实例服务所有进程"的既定设计初衷——这个设计本来就是为了模拟真实部署里一个实例服务很多会话的场景）。
