...
2. 上方始终展示所有 Agent 的时间条，不受下方 Agent 或事件类型筛选影响。下方默认选择主 Agent，每次只查看一个 Agent，并按 Run 分组；点击其他 Agent 的时间标记会自动切换下方选择、清除不匹配的类型筛选并定位事件。模型过程与工具/MCP 分行，同类事件只按真实时间重叠分配子行；缩放和窗口宽度不改变行结构。密集时间点会统一横向展开并保留分隔，精确时间可悬停查看。
