...
6. 左侧只显示执行过程，保留模型、工具、MCP、状态检查点及失败、等待、恢复等节点，不提供「全部记录」切换。内部会话更新、排队同步、轮次、响应边界和用量通知仍保留在导出中。列表按 Agent + Run 分组，同一 Run 只显示一个标题；记录自身的 contextId 仍精确关联调用，不能因视觉分组而混用上下文。Run 表示一次 Agent 执行，turn 是该 Run 内的轮次，# 是事件流序号。
