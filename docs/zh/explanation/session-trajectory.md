...
2. 每个 Agent 有独立条带组，模型过程与工具/MCP 分行；同类事件只按真实时间重叠分配子行，缩放和窗口宽度不改变行结构。密集时间点会统一横向展开，保留标记之间的分隔，必要时横向滚动；各 Agent 使用同一映射，刻度跟随映射显示，悬停和详情保留精确时间。点击标记或左侧事件行选择节点，可按类型或 Agent 筛选。
