...
1a. **Sub-agents stay ScienceDiscovery's `task`.** Each one runs as its own JiuwenSwarm session with all of ScienceDiscovery's tools, sandbox, approvals and sub-agent cards. JiuwenSwarm's own sub-agents cannot be used yet (0.2.6, the `agent` mode): its built-in `general-purpose` sub-agent is built with the parent's built-in tools and no MCP servers, so it never gets ScienceDiscovery's tools, and a custom agent (`agents.create`) fails to spawn (`'str' object has no attribute 'name'`: its tool list is names where the spawn path expects tool objects). `test/contract/jw-only/live.mjs subagent-probe` shows what one looks like. A subagent whose own JiuwenSwarm session needs a **second** turn — resumed later by an automatic wake, not answered synchronously inside the same `task` call — does not reliably get one: `issue-85-foreground-exec-inbox`'s third case (a subagent backgrounds a command, then is woken by its own result) and `journey-session-trajectory` (a subagent delegated mid-run, then the trajectory export) both leave a run stuck `running` past its terminal-state poll, or leave the wake's Project undeleted (`should be gone before the next journey starts`). Confirmed independent of test batching (each fails run alone). Not yet root-caused: candidates are the nested run's own idle/turn deadlines (now tracked the same way as the main run, see below) firing too late relative to the journey's own poll window, or JiuwenSwarm not resuming the subagent's session promptly on a wake-triggered continuation. Open.
...
5. **Wake notices: fixed.** The scripted model in `test/helpers/journeys.ts` now reads the user turn out of JiuwenSwarm's own envelope (`你收到一条消息：{"content": "..."}`) before matching it against `[Execution notifications]`; `issue-77-wake-notice` and two of `issue-85-foreground-exec-inbox`'s three cases pass. The third (subagent backgrounds work, then is woken by its own result) is the open item in 1a. Run the group with `.ci/run-e2e.sh mocked` (`CI_E2E_BACKEND=jiuwenswarm` is now the default). The journeys are load-sensitive on a small host: two of them failed once when run back to back on the 2-core server and passed alone (three repeats), so run them one at a time when in doubt.
...
8. **UT audited against a real JiuwenSwarm.** `services/api/src/server.test.ts` (the host UT tier's biggest suite) now runs its agent turns on a real JiuwenSwarm and adapter through `scripts/with-jiuwenswarm.sh` (`pnpm ci:ut:host` wires this in). The audit found:
   ...
   - **Real, JiuwenSwarm-specific behaviour the tests now account for, not bugs:** a Session's skills are not a selection (every installed skill is available everywhere; see "Sub-agents stay ScienceDiscovery's `task`" above for the sub-agent equivalent — every deferred tool, connector MCP tools included, is promoted up front for a sub-agent too); skill-creator is loaded with JiuwenSwarm's `skill_tool`, not our `read_skill`; a tool's result, re-injected into the model's next turn, arrives wrapped in JiuwenSwarm's own Python-repr envelope (`{'result': '...'}`, not JSON) rather than verbatim.
   - **A second, unrelated bug in the same sweep, fixed:** the suite's own inline model stub for background-execution wake notifications did not unwrap JiuwenSwarm's envelope around the user turn before matching `[Execution notifications]` — the same class of bug already fixed in `test/helpers/journeys.ts` for the E2E journeys.
   - **Known, still open:** three tests that switch a session's approval mode (to `always_allow`, and to `ask`) *while a run is in progress* fail; not yet root-caused, and distinct from the approval-resource bug above (that one is fixed). Treat mid-run approval-mode switching as unverified on this executor until this is resolved.
