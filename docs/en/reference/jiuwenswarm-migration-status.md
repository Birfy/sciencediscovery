...
11. **UT audited against a real JiuwenSwarm.** `services/api/src/server.test.ts` (the host UT tier's biggest suite) now runs its agent turns on a real JiuwenSwarm and adapter through `scripts/with-jiuwenswarm.sh` (`pnpm ci:ut:host` wires this in). The audit found:
    ...
    - **Known, still open:** three tests that switch a session's approval mode (to `always_allow`, and to `ask`) *while a run is in progress* fail; not yet root-caused, and distinct from the two approval bugs above (both fixed). Treat mid-run approval-mode switching as unverified on this executor until this is resolved.
