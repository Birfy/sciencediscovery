...
- JiuwenSwarm keeps each agent's conversation (main agent, and each subagent) in its own session, and compresses it when it reaches 80% of `JIUWENSWARM_CONTEXT_WINDOW_TOKENS`. JiuwenSwarm's own model calls (the summaries it writes when compressing, titles for sessions) use its *default model*: the adapter makes that entry point at itself (`sd-default`), and it sends those calls to the model of the run in progress. The JiuwenSwarm instance is therefore ScienceDiscovery's own; do not share it with other work. The system prompt is ScienceDiscovery's own (workspace, governance, skills, specialist and, when there is one, the run contract): the adapter puts it on every model request in place of JiuwenSwarm's. ScienceDiscovery's per-step context (plan snapshot, durable state) is **not** injected, and JiuwenSwarm still adds its own per-turn wrapper and dynamic context as user messages.
