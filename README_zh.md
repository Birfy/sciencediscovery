...
产品自己不带模型，用你的 API。点左侧栏底部的**系统配置**，两处要填：

1. **模型注册表** —— 选一个预置服务商，或手填**基础 URL**；填入 **API Key**，再把要用的模型加进来。
2. **全局默认值** —— 把刚加的模型设为**任务模型**。

字段含义和可以改用的环境变量，见[配置参考](docs/zh/reference/configuration.md)。
