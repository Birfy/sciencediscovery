...
import { entryTitle, internalEntry, recordGroups } from "./presentation.js";
...
export function TrajectoryViewer({ sessionId, title, port, locale, onClose }: {
  ...
  return <div className="trajectory-backdrop"><div className="trajectory-dialog" role="dialog" aria-modal="true" aria-label={tr("Session 轨迹", "Session trajectory")} ref={dialog} tabIndex={-1} onKeyDown={event => {
    ...
    {index && <>
      ...
      <nav className="trajectory-modes" aria-label={tr("记录视图", "Record view")}><button aria-pressed={!showInternal} onClick={() => setShowInternal(false)}>{tr("执行过程", "Execution")}</button><button aria-pressed={showInternal} onClick={() => setShowInternal(true)}>{tr("全部记录", "All records")}</button></nav>
      <div className="trajectory-event-list" ref={list}>{groups.length ? groups.map(group => <section className="trajectory-event-group" key={group.id}><header><strong>{index.agents.find(a => a.id === group.agentId)?.label}</strong><small title={group.runId}>Run {group.runId?.slice(0, 8) ?? "—"}{group.turn !== undefined ? ` · turn ${group.turn}` : ""}{group.invocation !== undefined ? ` · ${tr("调用", "Invocation")} ${group.invocation}` : ""}</small></header>{group.entries.map(e => <button key={e.id} data-event-type={e.eventType ?? e.label} aria-current={selected === e.id ? "true" : undefined} onClick={() => choose(e)} style={{ "--event-color": colors[e.kind] } as CSSProperties}><span><i />{labels[e.kind]}<time>{e.timestamp ? time(e.timestamp) : tr("时间未记录", "Time not recorded")}</time></span><strong>{entryTitle(e, zh)}</strong><small className="trajectory-run" title={e.runId}>{e.turn !== undefined ? `turn ${e.turn}` : ""}{e.sequence !== undefined ? ` · #${e.sequence}` : ""}{!e.timestamp ? tr(" · 调用记录（非过期版本）", " · Invocation record (not superseded)") : ""}</small></button>)}</section>) : <p>{tr("暂无匹配的记录", "No matching records")}</p>}</div></aside>
      ...
      {detail ? <><div className="trajectory-detail-heading"><strong>{labels[detail.entry.kind]} · {entryTitle(detail.entry, zh)}</strong><time>{detail.entry.timestamp ?? tr("时间未记录；这是该次调用的固定记录，并不表示已过期。", "Time not recorded; this is the fixed record of this invocation, not a superseded version.")}</time><small className="trajectory-run">Run {detail.entry.runId ?? "—"}{detail.entry.turn !== undefined ? ` · turn ${detail.entry.turn}` : ""}{detail.entry.sequence !== undefined ? ` · #${detail.entry.sequence}` : ""}</small></div>
