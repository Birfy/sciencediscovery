...
export function TrajectoryViewer({ sessionId, title, port, locale, onClose }: {
  ...
  const associatedInput = index?.entries.find(e => e.kind === "input" && e.agentId === detail?.entry.agentId && e.runId === detail?.entry.runId && !!e.contextId && e.contextId === detail?.entry.contextId);
  useEffect(() => {
  ...
  return <div className="trajectory-backdrop"><div className="trajectory-dialog" role="dialog" aria-modal="true" aria-label={tr("Session 轨迹", "Session trajectory")} ref={dialog} tabIndex={-1} onKeyDown={event => {
    ...
    {index && <>
      ...
      <section className="trajectory-detail"><nav aria-label={tr("详情类型", "Detail type")}>{[["event", tr("事件内容", "Event content")], ["state", tr("Agent 状态", "Agent state")]].map(([id, label]) => <button key={id} aria-pressed={tab === id} onClick={() => setTab(id!)}>{label}</button>)}</nav>
      {detail && detail.entry.id === selected ? <><div className="trajectory-detail-heading"><strong>{labels[detail.entry.kind]} · {entryTitle(detail.entry, zh)}</strong><time>{detail.entry.timestamp ?? tr("时间未记录", "Time not recorded")}</time><small className="trajectory-run">Run {detail.entry.runId ?? "—"}{detail.entry.turn !== undefined ? ` · turn ${detail.entry.turn}` : ""}{detail.entry.sequence !== undefined ? ` · #${detail.entry.sequence}` : ""}</small>{detail.entry.kind !== "input" && associatedInput && <button className="trajectory-input-link" onClick={() => choose(associatedInput)}>{tr("查看本次输入", "View request input")}</button>}</div>
