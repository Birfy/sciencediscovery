...
export function TrajectoryViewer({ sessionId, title, port, locale, onClose }: {
  ...
  const [mode, setMode] = useState<"events" | "history">("events");
  ...
  useEffect(() => {
    ...
    port.index(sessionId, controller.signal).then(value => {
      setIndex(value);
      const all = [...value.entries, ...(value.historicalEntries ?? [])];
      setSelected(current => all.some(e => e.id === current) ? current : value.entries.find(e => e.kind === "input")?.id ?? all[0]?.id);
      if (!value.entries.length && value.historicalEntries?.length) setMode("history");
  ...
  const timelineEntries = useMemo(() => index?.entries.filter(e => (filter === "all" || e.kind === filter) && (agent === "all" || e.agentId === agent)) ?? [], [index, filter, agent]);
  const entries = mode === "events" ? timelineEntries : (index?.historicalEntries ?? []).filter(e => (filter === "all" || e.kind === filter) && (agent === "all" || e.agentId === agent));
  ...
  const choose = (entry: TrajectoryEntry) => { setSelected(entry.id); setMode(entry.timestamp ? "events" : "history"); setError(""); };
  ...
  return <div className="trajectory-backdrop"><div className="trajectory-dialog" role="dialog" aria-modal="true" aria-label={tr("Session 轨迹", "Session trajectory")} ref={dialog} tabIndex={-1} onKeyDown={event => {
    ...
    {index && <>
      ...
      <nav className="trajectory-modes" aria-label={tr("记录视图", "Record view")}>{([["events", tr("事件时间线", "Events"), index.entries.length], ["history", tr("历史版本", "History"), index.historicalEntries?.length ?? 0]] as const).map(([id, label, count]) => <button key={id} aria-pressed={mode === id} onClick={() => { setMode(id); setSelected((id === "events" ? timelineEntries : index.historicalEntries)?.[0]?.id); }}>{label} ({count})</button>)}</nav>
      {mode === "history" && <p className="trajectory-history-notice">{tr("以下记录没有可靠的事件时间，不放入时间轴。", "These records have no reliable event time and are not placed on the timeline.")}</p>}
      <div className="trajectory-event-list" ref={list}>{entries.length ? entries.map(e => <button key={e.id} aria-current={selected === e.id ? "true" : undefined} onClick={() => choose(e)} style={{ "--event-color": colors[e.kind] } as CSSProperties}><span><i />{labels[e.kind]}<time>{e.timestamp ? time(e.timestamp) : tr("时间未知", "Time unknown")}</time></span><strong>{e.label}</strong><small>{index.agents.find(a => a.id === e.agentId)?.label}</small><small className="trajectory-run" title={e.runId}>Run {e.runId?.slice(0, 8) ?? "—"}{e.turn !== undefined ? ` · turn ${e.turn}` : ""}{e.sequence !== undefined ? ` · #${e.sequence}` : ""}</small></button>) : <p>{tr("暂无匹配的记录", "No matching records")}</p>}</div></aside>
      ...
      {detail ? <><div className="trajectory-detail-heading"><strong>{labels[detail.entry.kind]} · {detail.entry.label}</strong><time>{detail.entry.timestamp ?? tr("历史记录：时间未知", "Historical time unavailable")}</time><small className="trajectory-run">Run {detail.entry.runId ?? "—"}{detail.entry.turn !== undefined ? ` · turn ${detail.entry.turn}` : ""}{detail.entry.sequence !== undefined ? ` · #${detail.entry.sequence}` : ""}</small></div>
